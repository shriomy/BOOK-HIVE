...
const BorrowBookTable = () => {
  ...
  return (
    <div className="p-6 bg-white rounded-lg shadow-sm max-w-6xl mx-auto">
      ...
      {loading && <div className="text-center py-4">Loading book data...</div>}
      {error && !loading && (
            ...
            {filteredBooks.length === 0 ? (
              ...
              filteredBooks.map((book) => (
                <tr key={book.id} className="hover:bg-gray-50">
                  ...
                  <td className="py-4 px-6">
                    <button 
                      className={`flex items-center text-sm ${book.status === 'Returned' ? 'text-gray-400 cursor-not-allowed' : 'text-indigo-600 hover:text-indigo-800 cursor-pointer'}`}
                      disabled={book.status === 'Returned'}
                      onClick={() => book.status !== 'Returned' && generateReceipt(book.id)}
                    >
